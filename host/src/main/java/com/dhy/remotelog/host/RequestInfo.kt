...
class RequestInfo(request: Request) {
    ...
    val user: String?
    private val head_user = "_USER_"

    init {
        user = request.header(head_user)
        val url = request.url()
...
internal fun RequestInfo.toLog(response: String, httpCode: Int, costTimeMS: Long): RequestLog {
    val requestInfo = this
    val log = RequestLog()
    log.date = System.currentTimeMillis()
    log.costTimeMS = costTimeMS
